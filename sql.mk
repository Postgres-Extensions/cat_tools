...
# marker below. Not anchored -- a plain substring match -- so it also fires
# on the handful of @generated@ occurrences that coincidentally appear as
# trailing annotations on real code lines in cat_tools.sql.in, turning them
# into harmless (if odd-looking) extra comment text; that's fine, they're
# already inside a -- comment. That's what lets the copy rule below tag a
# version file's own @generated@ markers with a trailing "VERSIONED FILE!"
# (see there): this substitution doesn't need to know or care whether that
# tag is present, it just resolves the @generated@ prefix either way.
...
# This target already has a recipe: root control.mk (auto-generated by
# pgxntool from cat_tools.control, see pgxntool/base.mk) defines it as
# `cat sql/cat_tools.sql` with a generic one-line marker, no .sql.in layer
# and no _apply_version_seds. Since that's an explicit (non-pattern) rule
# for this exact target, GNU Make would use it over the sql/%.sql pattern
# rule above regardless of include order -- an explicit rule always beats
# a pattern rule for the same target. Overriding it here is the only way to
# route this target through the .sql.in / version-sed pipeline the rest of
# this file builds. GNU Make's "overriding recipe" warning for this target
# is expected.
