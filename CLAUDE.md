...
See [`../ai/CLAUDE.md`](../ai/CLAUDE.md) for the general version-specific
SQL file tracking rules (`.sql.in` vs. `.sql`, why version-specific install
scripts and update scripts are tracked, when a `stable`-style pseudo-version
is gitignored instead). cat_tools' own current pseudo-version
(`sql/cat_tools--stable.sql.in`) is gitignored per that last rule — see
`sql/.gitignore`'s comment and RELEASE.md step 4 for exactly why.
